...
```{r}
  ...
  clusterExport(cl, list("cSimL"))
  clusterExport(cl, list("fullDf"))
  
result<-foreach(i=uIds, .combine = rbind.data.frame, .export=c('cSimL', 'cOptimx', 'fullDf'), .packages = c("compiler", "optimx")) %dopar% {
  subDf <- subset(fullDf, subID == i) # Subset participant data
  output <- simL(subDf$trialTotalT1, subDf$Idx, subDf$feedback, 4, posGraph)
  output
...
fullDf2 <- cbind(fullDf, data.frame(VE=SimLDf$Value) )
library(lmerTest)

m1 <- lmer(scale(selfRespT1) ~ scale(VE) + ( scale(VE) | subID ) + ( 1 | Trait ), data = fullDf2, 
              control = lmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun=2e5)), REML = F)
summary(m1)
r2glmm::r2beta(m1)

paramDf <- matrix(ncol=4, nrow=length(uIds))
k<-0
for(n in 1:length(uIds) ){
  i <- uIds[n]
  subDf<-subset(fullDf2, subID==i)
  RSE <- sum((subDf$selfRespT1  - (subDf$VE))^2)
  
  paramDf[n, 1] <- i
  paramDf[n, 2] <- RSE
  paramDf[n, 3] <- nrow(subDf) * log(RSE/nrow(subDf)) + k * log(nrow(subDf)) # Compute BIC
  paramDf[n, 4] <- 2 * k + nrow(subDf) * log(RSE/nrow(subDf))
}
SimL.ParamDf <- paramDf
colnames(SimL.ParamDf) <- c("subID","LL","BIC","AIC")
SimL.ParamDf <- as.data.frame(SimL.ParamDf)
sum(SimL.ParamDf$AIC)
```
...
for(i in uIds){
  ...
  subDf <- subset(fullDf, subID == i) # Subset participant data
  #subOut <- cGraphMix_OutMod(subDf$trialTotalT1,subDf$Idx,subDf$feedback,4,posGraph,type="in",cue="single",c(LOO_LR.Pos, LOO_LR.Neg, LOO_Mix)) # Produce RL output with fitted parameters
  #subOut <- cGraphMix_FS(subDf$trialTotalT1,subDf$Idx,subDf$feedback,4,posGraph,type="in",cue="single",c(LOO_LR.Pos, LOO_LR.Neg, LOO_Mix, LOO_FS) )
  #subOut <- cGraphMix_FS_OutBias(subDf$trialTotalT1,subDf$Idx,subDf$feedback,4,posGraph,type="in",cue="single",fix=T,c(LOO_LR.Pos, LOO_LR.Neg, LOO_Mix, LOO_FS, LOO_Bias) )
  ...
  #subOut <- cGraphProp(subDf$trialTotalT1,subDf$Idx,subDf$feedback,4,posGraph,type = "in", cue = "single", modulate = c("feed", "no"), degP = "no", c(LOO_LR.Pos, LOO_LR.Neg) )
...
#names(LOO_Df)[2]<-"trialInCluster"
names(LOO_Df)[2]<-"trialTotalT1"
#names(LOO_Df)[ncol(LOO_Df)-2]<-"predError"
colnames(LOO_Df)[colnames(LOO_Df)=="RPE"] <- "predError"
#names(LOO_Df)[ncol(LOO_Df)-1]<-"valEst"
colnames(LOO_Df)[colnames(LOO_Df)=="Value"] <- "valEst"
#names(LOO_Df)[11]<-"socExp"
...
#write.csv(GraphMix.Ridge.ParamDf, "GraphMix.Ridge.ParamDf.csv", row.names = FALSE)
...
```{r}
# names(allCents)[1] <- "Trait"
# fullTDdf2 <- merge(fullTDdf, allCents[c("Trait","desirability","prevalence","category","interpersonal","observability","outDegree","inDegree","between","eigen","close","efficiency","transitivity","pageRank")], by = "Trait", all.x = TRUE)
# fullTDdf2 <- merge(fullTDdf2, indDiff, by = "subID", all.x = TRUE)
# tBt_Df <- merge(tBt_Df, allCents[c("Trait","desirability","prevalence","category","interpersonal","observability","outDegree","inDegree","between","eigen","close","efficiency","transitivity","pageRank")], by = "Trait", all.x = TRUE)
# tBt_Df2 <- merge(tBt_Df, indDiff, by = "subID", all.x = TRUE)
# fullTDdf2 <- fullTDdf2[order(fullTDdf2$subID, fullTDdf2$trialTotalT1),]
# tBt_Df2 <- tBt_Df2[order(tBt_Df2$subID, tBt_Df2$trialTotalT1),]
```
