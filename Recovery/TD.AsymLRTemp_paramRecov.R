TD.AsymLRTemp_paramRecov <- function(param){
  ...
  x = c('trialNum', 'w1','w2','w3','w4','w5','RPE','Value', 'socExp', 'feat', 'prob', 'valence', 'feedback')
  ...
  for (n in 1:N) { # for each trial in all of the trials
    ...
    TD.Out[n,] <- c(n, w[1], w[2], w[3], w[4], w[5], RPE, V, SE, S, trialProb, trialValence, rew)
  }
