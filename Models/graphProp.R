graphProp <- function(trialN, feat, rew, weight, graph, type, cue, modulate = c("feed","no"), degP = "no", param){
  library(igraph)
  library(purrr)
  library(compiler)
  ...
  learn <- function(weights, dist, lr, RPE, degP="no"){
  ...
  for (n in 1:N) { # for each trial in all of the trials
    ...
    dist<-distances(graph, v = S, mode= as.character(type) )
    w <- learn(w, dist, lr, RPE, degP = as.character(degP) )
