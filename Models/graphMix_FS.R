graphMix_FS <- function(trialN, feat, rew, weight, graph, type, cue, param){
  library(igraph)
  library(purrr)
  library(compiler)
  ...
  for (n in 1:N) { # for each trial in all of the trials
    ...
    feed = sigTrans(rew[n],slope=(1/fs), shift= 0 )
    feedList[n] <- feed
    ...
    if(n>1){
      ...
      ws <- apply(s, MARGIN = 2, function(x) sum(x[1:(n-1)] * feedList[1:(n-1)]) / sum(x[1:(n-1)]) ) # apply weighted sum to each column
      
      #}
      if(sum(curSim)!=0){
        # V_S = sum(curSim * rew[1:(n-1)]) / sum(curSim) # value estimate is weighted sum of current trait
        
        V_S = sum(curSim * feedList[1:(n-1)]) / sum(curSim) # value estimate is weighted sum of current trait
        
      }else if(sum(curSim)==0){
        V_S = mean(feedList[1:(n-1)])
      }
    }
    S_PE = feed - V_S
    ...
    RPE = feed - V_PE # reward prediction error is the reward of current trait minus the value estimate of current trait
