graphMixDec <- function(trialN, feat, rew, weight, graph, type, cue, param){
  library(igraph)
  library(purrr)
  library(compiler)
  ...
  x = c('LoopN', 'trialNum', paste0('wpe',1:148) ,'RPE','V_PE', 'S_PE', 'V_S', 'Value')
  ...
  for (n in 1:N) { # for each trial in all of the trials
    ...
    TD.Out[n, ] <- c(n, trial, wpe, RPE, V_PE, S_PE, V_S, VE)
  }
